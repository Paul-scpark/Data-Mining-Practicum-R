...
Everytime_df <- Everytime_df %>% 
  dplyr::mutate(content = gsub("[^가-힣]+", " ", content),      # 한글이 아닌 것들 제거
...
이번에는 2019년과 2020년의 봄과 가을 학기 그리고 여름과 겨울 방학으로 나누어서 게시글(POST)의 차이를 확인한다.

전반적으로 봄 학기에 글들이 많이 만들어진 것을 볼 수 있고, 그 다음으로는 여름방학, 가을학기, 겨울방학의 순이다.

상대적으로 봄 학기의 게시글들이 다른 시기보다 많았던 것을 확인할 수 있다.
...
그리고 댓글에 대한 것도 시기 별로 개수를 확인해보았다.

댓글 개수의 추이는 게시글들이 많을수록 그 댓글 수도 증가한 것을 볼 수 있다.

결과적으로 게시글과 댓글의 개수가 봄학기 - 여름방학 - 가을학기 - 겨울방학 순으로 많은 것을 확인할 수 있다.
...
그리고 00시부터 24시까지 학생들이 시간에 따라 얼마나 게시글을 작성하는지 그 분포를 확인해보고자 한다.

눈에띄게 확인할 수 있는 것은 일반적으로 취침시간이라고 할 수 있는 4시부터 9시까지는 게시글의 수가 확연히 감소하는 것을 볼 수 있다.

또한 저녁 8시부터 취침 전 약 새벽 3시까지 게시글의 개수가 가장 많은 것을 확인할 수 있다.
...
데이터가 충분히 있는 2019년과 2020년에 대해서 결과를 확인해보고자 한다. 감성사전과 우리 단어에 대해 join을 하려고 하다 보니, 단어가 완전히 일치하지 않으면 잘 묶이지 않는다는 한계가 있어서 NA의 개수가 많이 나온다. 그럼에도 불구하고, 현재 있는 약 4만개 데이터들의 분포를 확인해보면, 2019년에는 듬성듬성 데이터가 분포하는 것을 볼 수 있고, 2020년에는 전반적으로 감성 점수가 매치된 단어들의 비율이 조금 더 높은 것을 확인할 수 있다.
...
```{r}
# 8주차에서 긍정이 많이 나온 이유를 워드클라우드로 확인
df <- sentiment_df %>% 
  dplyr::filter((year(datetime) == 2019) | (year(datetime) == 2020)) %>% 
  dplyr::mutate(label = ifelse(date(datetime) < '2019-02-25', '2019 winter vacation', 
                               ifelse(date(datetime) < '2019-06-15', '2019 spring semester', 
  ...
  mutate(diff = difftime(datetime, as.Date('2020-03-01'), units = 'days'), 
         week = ceiling(diff / 7), 
         label = ifelse(score %in% c(-2, -1), 'negative', 'positive')) %>% 
  filter(week >= 1) %>% 
  filter(week == 8)
...
df %>% 
  group_by(word) %>% 
  count() %>% 
  left_join(df[, c('word', 'label')], by = 'word') %>% 
  acast(word ~ label, value.var = 'n', fill = 0) %>% 
  comparison.cloud(colors = c('darkred', 'darkcyan'), max.words = 300)
```

8주차에서 긍정적인 감성 점수가 더 높았던 이유를 확인해보기 위해 긍정과 부정에 대한 각 감성 단어들을 워드클라우드로 만들어보았다. 시험기간이여서 힘들 것 같았던 8주차에 긍정적인 감성이 높은 이유를 확인해보고자 했는데, 아니다, 없다, 힘들다 등과 같은 부정적인 단어들도 생각보다 많이 나오고 있음을 직관적으로 확인할 수 있었다. 해당 결과로는 정확한 이유를 유추하는데 약간의 한계가 있는 것 같았다.