...
불러온 데이터에 NA가 있는지를 확인해봤더니, Script 변수에 11개의 NA가 있음을 확인하여 이를 출력해보았다.

그래서 마태복음, 마가복음, 누가복음, 요한복음에 대해 실제 성경을 찾아보니 원래 비어있음을 확인할 수 있었다.

(Text file 안에서 Enter의 구분이 잘못된 부분이 있었는데, 이는 직접 텍스트 파일(txt)을 열람해서 직접 고쳤다.)

### 1-2. Tokenization을 수행하고, 빈도분석 및 frequent word에 대한 시각화를 수행하여라. (+해석하기)
```{r}
...
bible_word %>% 
  ...
  ggtitle("Most Frequent Words in the Bible")+
  coord_flip() +   
  theme(plot.title = element_text(hjust=0.5))
```

아무래도 성경의 주인공은 하나님이기 때문에 하나님 관련 단어인 lord와 god가 많이 나오는 것을 당연하게 볼 수 있다. 또 구약의 상당한 부분이 하나님의 백성이 이스라엘에 관한 내용이기 때문 israel, people, land가 나오고 하나님은 우리르 그의 아들과 아들들이라고 부르기 때문에 son과 sons가 많이 나오는 걸 볼 수가 있다. 마지막으로 모세가 성경에서 많은 내용을 차지하는걸 볼 수가 있다.
...
tidy_bible %>% 
  ...
  ggplot(aes(x = strsplit(word, split('_'))[[1]][1], y = n, fill = label)) +
  geom_col(show.legend = F) +
  facet_wrap(~label, scales = 'free_y') +
  coord_flip() + xlab(NULL) + ylab(NULL)

### 2-1. Genesis부터 Malachi까지는 구약성서(Old testament)이고 Matthew부터는 신약성서(New Testament)이다. 신약과 구약에 대해 따로 빈도 분석을 수행하고, frequent words가 어떻게 다른지 비교하여라
```{r}
...
### 2-3. 신약과 구약 모두에서 10회 이상 나타나는 단어들을 추출하여라. 이 단어들의 출현 비율( 해당 단어의 빈도 / 데이터 내 모든 단어의 수 )을 계산하라. (신약/구약 대해서 각각 따로) 해당 단어의 신약성경에서 출현비율 (prop A)와 구약성경에서의 출현비율 (prop B)를 각각 계산한 후 log(propA / propB) = log ratio를 계산하여 log ratio top 20개 단어와 bottom 20개 단어를 추출하여보라. 이를 통해 알게된 사실을 설명하여라
```{r}
...
head(new_old_word,20)
tail(new_old_word,20)
```
...
bind_rows(head(new_old_word, 20) %>% mutate(label = 'top_20'), 
  ...
  ggplot(aes(x = reorder(word, -log_ratio), y = log_ratio, fill = label)) +
  geom_col(show.legend = F) +
...
- 신약에서 많이 나왔던 단어들은 faith, truth, teaching, teacher, crowd 등으로 군중들에 대하여 믿음에 대한 가르침에 대해 주로 이야기한 것으로 생각된다. 그 중에서도 faith와 crowd는 구약 보다 100배 많고, 나머지 단어는 10배 정도이다. 그 이유는 밑이 10인 log로 ratio를 구했기 때문이다. (log10^2) = 2
- 구약에서 많이 나왔던 단어들은 judah, israel, babylon, egypt, king 등으로 각 나라들에 대한 이야기로 내용을 풀어간 것으로 생각된다. 구약에 단어가 많아서 대체적으로 1000배 이상 많고, 그중에서도 judah랑 israel은 10000배 가까이 된다. 이스라엘과 유대민족에 관한 이야기라는 것을 명확하게 알 수 있다.
...
count_sent <- tidy_bible %>%
  inner_join(get_sentiments("nrc")) %>% 
  count(label, sentiment) 
```

마지막으로는 긍정과 부정 외에도 기쁨, 놀람 등의 다양한 감정을 가지고 있는 nrc 사전에 대해서도 감성 분석을 실시해본다.

그 결과 구약과 신약 모두 긍정과 믿음, 부정 등이 가장 높게 나오는 것을 볼 수 있었다.

### 3-2. 신약과 구약에서 나타나는 감성단어의 빈도를 비교
```{r}
bible_sentiment %>% 
  arrange(desc(n)) %>% 
  filter(label == 'old') %>% 
...
bible_sentiment %>%
    ...
    geom_col(show.legend = F) +
    facet_wrap(vars(sentiment, label), scales = "free_y") +
    coord_flip() +
    scale_x_reordered()
```

또한 구약과 신약 각각에 대하여 긍정과 부정 단어의 개수가 많은 것들을 출력하여 시각화 해보려고 한다.

신약에서는 dead, death, sin, evil 등과 같이 죽음과 죄에 관련된 것들이 가장 부정적인 것으로 확인된다. 또한 긍정적인 것은 faith, heven, love, holy, glory, grace 등이 있다.

구약에서는 신약과 비슷하게 sin, wicked, evil, death, die 등이 가장 부정적이었고, gold, holy, love, praise 등이 가장 긍정적인 단어들로 확인된다.

```{r}
count_sent %>% 
  group_by(label) %>% 
  mutate(prop_n = case_when(label == "new" ~ n/sum(n)*100,
                          TRUE~ n/sum(n)*100),
         word = reorder_within(sentiment,n, label )) %>% 
  ggplot(aes(x= reorder(sentiment, desc(prop_n)), y = prop_n, fill = label))+
  geom_col()+
  facet_wrap(~label, scales = "free_x")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  labs(y = 'Proportion of the sentiments',
       x = NULL)+
  scale_x_reordered()
```
...
### 3-3. 각 성경(Book) 별로 감성을 분석하여 비교하여라. 긍정적인 감성의 성경과 부정적인 감성의 성경은 어떤 것들이 있는가
```{r}
bible_sentiment <- tidy_bible %>% 
  inner_join(get_sentiments('bing')) %>% 
...
book_count_neg %>%
    top_n(10, wt = ratio) %>%
    mutate(Book = reorder(Book, ratio)) %>%
    ggplot(aes(Book, ratio, fill = ratio)) +
    geom_col(show.legend = FALSE) +
    scale_fill_gradient(low="orange", high = "red") +
    coord_flip() +
    ggtitle("Negative Books")
```

그렇다면, bing 사전을 통해 부정적인 책들은 무엇이 있는지를 조금 더 자세히 확인해보도록 한다.

따라서 bing 사전에서 부정적인 감성을 담고 있는 단어들을 바탕으로 성경의 책(Book) 별로 개수를 구해주도록 한다.

그 후에 각 책 별로 전체 단어들의 개수에 대해 나눠주는 방식으로 ratio 라는 새로운 변수를 만들어 준다.

이 변수는 전체 단어들 중에서 부정적인 감성을 가지고 있는 단어의 비율이 얼마나 되는지를 의미하는 변수이다.

이를 통해 시각화를 해보면, Titus, Habakkuk, Jude 등의 순서라는 것을 알 수 있다.
...
book_count_pos %>%
    ...
    ggplot(aes(Book, ratio, fill = ratio)) +
    geom_col(show.legend = FALSE) +
    scale_fill_gradient(low="dark green", high = "green") +
    coord_flip() +
    ggtitle("Positive Books")
```

동일한 방식으로 긍정적인 단어의 비율이 높았던 책들은 무엇인지 확인해보도록 한다.

결과를 확인해보니, Philemon, John 3, Titus, Philippians 등의 순서가 나오는 것을 볼 수 있다.

```{r}
senti_book <- tidy_bible %>% 
  inner_join(get_sentiments("bing")) %>% 
  count(Book, sentiment, sort = TRUE) %>% 
  spread(sentiment, n, fill = 0) %>% 
  mutate(sentiment = positive-negative) 

senti_book2<- senti_book %>% 
  mutate(po_or_ne= case_when(sentiment > 0 ~ 'positive', TRUE ~ 'negative'),
         po_or_ne = as.factor(po_or_ne)) 
```

이번에는 긍정 단어의 개수에서 부정 단어의 개수를 빼는 방식으로 각 책 별로 감성 점수(sentiment)를 계산해보도록 한다.

그리고 그 점수가 0 보다 크다면, 긍정을 작다면, 부정을 의미하는 Label을 만들어주도록 한다.

최종적으로는 각 책의 감성(sentiment) 값을 전체 감성 단어의 수(positive + negative)로 나누어 비율의 형태로 값을 표현할 수 있는 sentiment_prop이라는 새로운 열을 만들도록 한다.

```{r}
top10 <- senti_book2 %>% 
  top_n(10, sentiment) 

bottom10 <- senti_book2 %>% 
  top_n(-10, sentiment)

total10<- rbind(top10, bottom10)

total10 %>%
  ggplot(aes(x = reorder(Book, sentiment), sentiment, fill = Book)) +
  geom_col(show.legend = FALSE)+
  facet_wrap(~ po_or_ne, scales = "free_x")+
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  labs(y = 'sentiment_prop', x = 'Book')
```

그리고 각 감성 별로 높고, 낮은 10가지 책들을 추출하여 sentiment_prop을 Y축으로 하고, Book을 X축으로 하는 시각화를 만들어본다.

그래프를 확인한 결과, positive한 감성의 단어가 많이 출현한 Book 중 상위 10개에는 “1 Chronicles”,“1 Peter”,“1 Timothy”,“2 Corinthians”, “Colossians”, “Daniel”, “Ephesians”, “Hebrews”, “Philippians”, “Psalms”, “Song of Songs” 가 있다.

negative한 감성의 단어가 많이 출현한 Book 중 상위 10개에는 “1 Samuel”, “Deuteronomy”, “Ezekiel”, “Genesis”, “Isaiah”, “Jeremiah”, “Job”,“Leviticus”, “Numbers”, “Proverbs”이다.

### 3-4. 각 Chapter들을 감성분석하여보라. 창세기, 역대서, 열왕기, 시편 등 길이가 상대적으로 긴 성경을 일부 선정하여, 각 성경이 Chapter의 흐름에 따라서 감성이 어떻게 변화하는 지 분석하고, 본인들이 알고 있는 성경 지식과 대조하며 설명하여라.
```{r}
...
```{r}
tidy_bible$Chapter <- as.integer(tidy_bible$Chapter)

Kings <- tidy_bible %>% 
  filter(Book == 'X1.Kings' | Book =='X2.Kings') %>% 
  arrange(Book, Chapter) %>% 
  inner_join(get_sentiments("nrc")) %>% 
  filter(sentiment %in% c("positive","negative","disgust", "anticipation", "anger")) %>% 
  group_by(Book, Chapter) %>% 
  count(sentiment)
```

이번에는 나름의 스토리가 있다고 알려진 열왕기상과 열왕기하에 대해 조금 더 다양한 감성을 확인할 수 있도록 nrc 사전으로 같은 작업을 수행해보려고 한다.

이를 위해서 가장 먼저 Chapter 순서대로 정리하기 위해 integer 타입으로 변환해주도록 한다.

그 후, 열왕기상과 열왕기하 만을 선택하고, 빈도의 변화를 알고 싶은 5가지 sentiment(anger, anticipation, negative, positive, disgust)를 선택하여 Book, Chapter 별로 sentiment를 count 해주도록 한다.
...
Kings %>% 
  ...
  group_by(sentiment) %>% 
  ggplot(aes(Chapter, prop, col = sentiment, fill = sentiment)) + 
  geom_point()+
  facet_wrap(~ Book, scales = "free", nrow = 2)+
  stat_smooth()+
  labs(y = 'sentiment prop', x = 'Chapter')
```

그 후에 열왕기상과 열왕기하를 facet_wrap으로 하여 각 감정과 Chapter 별로 그 점수를 시각화해보았다.

가장 먼저 열왕기상은 유다와 이스라엘 왕들의 기록으로 하나님의 말씀에 순종하는 것이 나라에 이롭다는 것을 이야기한다.

따라서 전반적으로 부정보다는 긍정 감성이 높은 것을 가장 먼저 확인할 수 있다.

이후 열왕기상 후반부에서는 후대 왕국들이 죄를 범하고, 몰락했다는 내용들을 소개하는데 이에 따라 긍정은 줄어들고, 부정이 증가하고 있음을 볼 수 있다.

열왕기하는 열왕기상과 마찬가지로 왕들을 소개하며, 그 추세는 열왕기상과 유사함을 확인할 수 있었다. 

긍정과 부정 외에 다른 다양한 감성들을 통해 열왕기상과 열왕기하를 분석해보려고 했는데, 아쉽게도 생각보다 다른 감성들의 점수가 크게 변화하지는 않았다.